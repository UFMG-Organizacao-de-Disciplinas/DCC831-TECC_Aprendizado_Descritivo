...
- A tarefa de descoberta de subgrupos, em certa medida, generalizou a tarefa de mineração de padrões frequentes incorporando um atributo alvo à busca pelos padrões interessantes
  - [JV] Não só recorrentes, mas que também induzem grupos
...
- Em algumas situações, a excepcionalidade do subgrupo é percebida somente através de um modelo ajustado sobre os dados
  - [JV] Exemplo: casa grande tem mais consumo energético do que casa pequena. Porém a casa grande pode ter gato.
...
- [JV] Dúvida: na imagem da direita, não deveria haver um espaço vazio por onde passa o grupo da direita?
  - Resposta do professor: Sim, ou existem mais variáveis não sendo mostradas. Então talvez haja divergência em outra dimensão.
...
- [JV] Descritor é composto por um conjunto de seletores
  - Queremos encontrar descritores que separem a base de dados em dois grupos onde um deles tem correlação positiva e o outro não.
  - Se estiver usando classes, pode-se usar entropia

#### EMM

- Formalmente, os objetos do conjunto de dados agora é descrito por dois conjuntos de atributos:
  - Um conjunto $\mathscr{A}$ de atributos descritivos (não-alvo); $|\mathscr{A}| = q$
    - Numérico ou categórico
  - Um conjunto $\mathscr{L}$ de atributos alvo (rótulos) dos objetos; $|\mathscr{L}| = m$
...
- [JV] o $p$-value é só usado como método de separação, não necessariamente garante que ele é estatisticamente relevante.
...
- Considere a figura ao lado, as três retas (pontilhada, tracejada e sólida) representam modelos de regressão linear ajustados na população, bolinhas e quadradinhos, respectivamente
  - [JV] Se fizermos regressão linear só das bolinhas, teríamos uma reta de mesma inclinação dos quadrados. Comparando entre os complementos, eles não são interessantes, mas se comparar com a população, passam a ser.
...
- Além disso, a própria medida de qualidade pode ter um custo computacional alto por ser necessário computar algum valor sobre o grupo
  ...
  - [JV] O custo de se calcular a medida de qualidade já foi um gargalo
...
- Uma medida de qualidade associada poderia ser a diferença absoluta das correlações
  - $\phi_{abs}(X) = |r_{X} - r_{C}|$
    - [JV] Correlação no grupo menos correlação no complemento
      - Essa medida de qualidade intensifica grupos pequenos
      - Esse tipo de métrica geralmente não é boa
...
- Podemos ajustar pelo tamanho do grupo com um fator $n$ ou $\sqrt{n}$ como feito em SD
  - [JV] Num caso em que multiplica-se essa raiz com o $\phi$, o n vai num caminho e os $r$ vão em outro.
...
- Uma forma mais sofisticada é ajustar pela entropia, que favorece partições mais equilibradas
  ...
  - $H(X) = - \sum p(X) \log p(X) - \sum p(C) \log p(C)$, $C$ é o grupo com o qual comparamos (população ou complemento) - [JV] Agora passamos a usar a entropia como multiplicador do $\phi$ e isso tende a ser melhor que a raiz.
...
- Isso envolve uma transformação nos coeficientes chamada de transformação de Fisher
  - $z = \frac{1}{2} \ln \frac{1+r}{1-r}$
    - [JV] Isso converte o coeficiente em uma correlação normal
...
- A literatura estatística novamente nos informa que o teste $t$ pode ser usado com a estatística e graus de liberdade:
  - $t' = \frac{\hat{\beta}_X - \hat{\beta}_C}{ \sqrt{s_X^2 + s_C^2} }$ e $df = \frac{ (S_X^2 + S_C^2)^2 }{ \frac{S_X^4}{n-2} + \frac{S_C^4}{n_C{-2}} }$
    - [JV] Ele usa os coeficientes e as variâncias
...
- [Duivesteijn, W., Feelders][2016_Duivesteijn], A.J. & Knobbe, A. Exceptional Model Mining. Data Min Knowl Disc 30, 47–98 (2016)
